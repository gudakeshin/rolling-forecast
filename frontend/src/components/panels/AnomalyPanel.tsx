...
function FindingTypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className="text-[10px] px-1.5 py-0.5 rounded bg-surface-700/80 text-surface-300 font-mono">
      {label}
    </span>
  );
}
    ...
    <div className="mb-4 space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="bg-surface-800 rounded-lg p-3 border border-surface-700/50">
          <div className="text-[10px] text-surface-500 uppercase tracking-wider mb-1">Total Issues</div>
          <div className="text-xl font-bold text-white">{summary.total_anomalies}</div>
          <div className="text-[10px] text-surface-500">across {summary.total_line_items} line items</div>
        </div>
        <div className="bg-surface-800 rounded-lg p-3 border border-red-500/20">
          ...
          <div className="text-[10px] text-surface-500">{formatCurrency(summary.critical_value)} at risk</div>
        </div>
        <div className="bg-surface-800 rounded-lg p-3 border border-amber-500/20">
          ...
          <div className="text-[10px] text-surface-500">{formatCurrency(summary.warning_value)} to review</div>
        </div>
        <div className="bg-surface-800 rounded-lg p-3 border border-sky-500/20">
          <div className="text-[10px] text-sky-400 uppercase tracking-wider mb-1">Info</div>
          <div className="text-xl font-bold text-sky-400">{summary.info_count}</div>
          <div className="text-[10px] text-surface-500">low priority</div>
        </div>
      </div>
      ...
      {typeChart.length > 0 && (
        <div className="bg-surface-800 rounded-lg p-3 border border-surface-700/50">
          <div className="text-[10px] text-surface-500 uppercase tracking-wider mb-2">Issue Type Distribution</div>
            ...
            {typeChart.map((t) => {
              ...
              return (
                <div key={t.type} className="flex items-center gap-2">
                  <div className="text-[10px] text-surface-400 w-32 truncate">{t.type}</div>
                  ...
                  <span className="text-[10px] text-surface-400 w-6 text-right">{t.count}</span>
                </div>
              );
            })}
          </div>
    ...
    <div className="mb-3 space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2 py-1.5 text-[10px] rounded-lg border transition-colors ${filters.showDismissed
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <select
          ...
          className="text-[10px] bg-surface-800 border border-surface-700 rounded-md px-2 py-1 text-surface-300 focus:outline-none focus:border-deloitte-green/50"
        >
...
function FilterChip({ label, active, onClick, color }: { label: string; active: boolean; onClick: () => void; color?: string }) {
  return (
    <button
      onClick={onClick}
      className={`text-[10px] px-2 py-1 rounded-full border transition-colors font-medium ${active
    ...
    <div className={`border rounded-lg transition-colors ${item.is_dismissed
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] text-surface-400 truncate mt-0.5">
            {topFinding.headline}
          </div>
        </div>
        ...
        <div className="text-right flex-shrink-0 w-16">
          ...
          <div className="text-[9px] text-surface-500">{item.category}</div>
        </div>
      ...
      {isExpanded && (
        <div className="px-3 pb-3 border-t border-surface-700/50 mt-0">
          ...
          <div className="mt-3 space-y-2">
            <div className="text-[10px] text-surface-500 uppercase tracking-wider font-semibold flex items-center gap-1">
            ...
            {item.findings.map((f, i) => (
              <div key={i} className={`p-2.5 rounded-lg border ${SEVERITY_BG[f.severity]} bg-opacity-50`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[11px] font-medium">{f.headline}</span>
                  <FindingTypeBadge type={f.type} />
                </div>
                <p className="text-[10px] opacity-80 leading-relaxed">{f.detail}</p>
                <div className="flex items-center gap-3 mt-1.5 text-[9px] opacity-60">
          ...
          {item.sparkline.length > 0 && (
            <div className="mt-3">
              <div className="text-[10px] text-surface-500 uppercase tracking-wider font-semibold flex items-center gap-1 mb-1">
              ...
              <div className="flex items-center gap-4 mt-1 text-[9px] text-surface-500">
          ...
          {/* Inline override */}
          <div className="mt-3">
            <div className="text-[10px] text-surface-500 uppercase tracking-wider font-semibold flex items-center gap-1 mb-1.5">
            ...
            {!editMode ? (
              <button
                ...
                className="flex items-center gap-1.5 text-[10px] text-deloitte-green hover:text-deloitte-green/80 transition-colors"
              >
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 text-[10px] bg-deloitte-green text-surface-900 rounded font-medium hover:bg-deloitte-green/80 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 text-[10px] text-surface-400 hover:text-white rounded border border-surface-700 hover:border-surface-600 transition-colors"
                  >
                    <X className="w-3 h-3" /> Cancel
                  </button>
                  {editReason.length > 0 && editReason.length < 10 && (
                    <span className="text-[9px] text-amber-400">{10 - editReason.length} more chars</span>
                  )}
                </div>
              </div>
            )}
          ...
          {/* Meta info */}
          <div className="mt-3 flex flex-wrap items-center gap-3 text-[9px] text-surface-500">
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[9px] opacity-60 truncate">{action.detail}</div>
      </div>
